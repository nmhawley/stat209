...
```{r}
yelp_model <- lm_basic(yelp ~ 1, data = burrito)
reg_table(yelp_model, level = 0.95)
```
...
**Answer**: Even the lowest value in the confidence interval is above 3.7, so we can infer that burrito restaurants overall have higher Yelp rating than other restaurants in the area.
...
**Answer**: Yelp review scores are the average of all Yelp reviewers.

Now fit a linear regression that predicts the yelp score as a function of
whether the burrito includes chips.

```{r}
yelp_chips <- lm_basic(formula = yelp ~ 1 + chips_included, data = burrito)
reg_table(yelp_chips, level = 0.95)
```
...
**Answer**: No, the value is much closer to 0 than 1 meaning that there is a lot of variation in the data.
...
```{r}
yelp_synergy <- lm_basic(formula = yelp ~ 1 + high_synergy, data = burrito)
reg_table(yelp_synergy, level = 0.95)
```
...
```{r}
overall_synergy <- lm_basic(formula = overall ~ 1 + high_synergy, data = burrito)
reg_table(overall_synergy, level = 0.95)
```
