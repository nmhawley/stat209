...
**Answer**: 369 seconds at the third percentile and 4107.46 seconds at the 97th percentile.
...
```{r}
model_perfectionism <- lm_basic(perfectionism ~ 1 + speed,  data = pf)
reg_table(model_perfectionism, level = 0.95)
```
...
**Answer**: Both slopes are significant and each increase by about the same amount, so the users who completed the test in normal time and slow time have a similar average perfectionism score, but the fast test-takers took have lower scores.
...
model_perfectionism_norm <- lm_basic(perfectionism ~ 1 + speed_normal,  data = pf)
reg_table(model_perfectionism_norm, level = 0.95)
```
...
**Answer**: No, the slope for the slow test-takers is not significant
...
**Answer**: Some people may finish quickly if they are not actually reading the questions and just selecting random answers, and some people may take a long time because they just left the webpage open and came back and finished the test later.
...
```{r}
model_warmth <- lm_basic(warmth ~ 1 + tension + social_boldness, data = pf)
...
**Answer**: They are both significantly positive.

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model_warmth <- lm_basic(warmth ~ 1 + tension + social_boldness + speed + gender + fct_lump(country, 5), data = pf)
reg_table(model_warmth, level=0.95)
```

Does this change the slopes for your two traits much?

**Answer**: The slopes for the traits changed only slighty and are both still significantly positive.

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**: There is a positive correlation between warmth with both tension and social boldness even when taking into account speed, gender, and country.