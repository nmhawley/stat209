...
**Answer**: Although the variable is numeric, it is not continuous and behaves almost like a categorical variable because the only possible values are integers 1 through 20.

Produce a confidence interval for the mean of your trait.

```{r}
model_warmth <- lm_basic(warmth ~ 1, data = pf)
reg_table(model_warmth, level = 0.95)
```
...
model_warmth_hu <- lm_basic(warmth ~ 1, data = hungary)
reg_table(model_warmth_hu, level = 0.95)
```

What do you notice about this confidence interval compared to the original
one? Can you explain why this is the case?

**Answer**: It is wider because there are less observations in the Hungary dataset, so we can be less certain about the true mean.
...
```{r}
model_warmth_hu_gender <- lm_basic(formula = warmth ~ 1 + gender, data = hungary)
reg_table(model_warmth_hu_gender, level = 0.95)
```
...
**Answer**: No, the confidence interval for the change contains zero, so it is not statistically significant.
...
```{r}
age <- filter(pf, age >= 30)
age_30_49 <- filter(age, age<=49)
```
...
```{r}
age_30_49$fourties <- age_30_49$age>=40
```
...
```{r}
model_warmth_age <- lm_basic(warmth ~ 1 + fourties, data = age_30_49)
reg_table(model_warmth_age, level = 0.95)
```
...
**Answer**: Yes, there is a statistically significant positive change for the warmth personality trait for people in their fourties versus people in their thirties. I think there is a stereotype that people traditionally get less friendly as they age, but this is not the case according to this data.
...
```{r}
model_warmth_lumped <- lm_basic(warmth ~ 1 + fct_lump(country, 5), data = pf)
reg_table(model_warmth_lumped, level = 0.95)
```

Using the previous model, which country (not including "Other") has the
highest score for your trait? Which has the lowest?

**Answer**: The US has the highest score because it has the smallest negative change, and India has the lowest score.
...
```{r}
model_sensitivity_baseline <- lm_basic(sensitivity ~ 1 + baseline, data = pf)
reg_table(model_sensitivity_baseline, level = 0.95)
```
...
```{r}
model_sensitivity_baseline_gender <- lm_basic(sensitivity ~ 1 + baseline + gender, data = pf)
reg_table(model_sensitivity_baseline_gender, level = 0.95)
```

Describe the slope for the gender term in words:

**Answer**: Sensitivity decreases by an average of -0.5495 for males.

Add predictions from the previous model to `pf`.

```{r}
pf <- add_prediction(pf, model_sensitivity_baseline_gender)
```
...
```{r}
ggplot(pf, aes(model_sensitivity_baseline_gender_pred, baseline)) +
  geom_point(aes(color = gender))
```

What do the predicted values look like?

**Answer**: The predicted values for males have a higher intercept than females.
