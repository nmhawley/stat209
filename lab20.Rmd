...
**Answer**: The slope coefficient indicates that for each carat that a diamond increases, the price will go up by 7756 dollars.
...
**Answer**: Yes, because the confidence interval is relatively small.

Would you be surprised if we collected another set of diamonds, in exactly the
same way as before, and found a slope estimate equal to 7500?

**Answer**: Yes, because this is saying that the price of a diamond of zero carats would be 7500 dollars.
...
**Answer**: If the confidence interval contains only positive or only negative intervals, we can be confident about whether or not the value will increase as the other variable increases.
...
**Answer**: For each millimeter of depth that the diamond increases, the price will decrease by 29.65.
...
**Answer**: Values that are only negative

Interpret this in words (your words, not just the formal definition).

**Answer**: Since the values are only negative that means that we can be confident that an increase in depth will cause a decrease in price.
...
**Answer**: Yes, the depth is negative, and the carats is positive.

Use the `add_prediction` function to add predictions from this model
back into the diamonds dataset.

```{r}
diamonds <- add_prediction(diamonds, model_depth)
diamonds
```
...
**Answer**: If two diamonds have the same weight in carats, the one that is one more millimeter in depth will cost 102.2 dollars less, and if two diamonds have the same depth, the one that weighs one carat more will cost 7765.1 more.
...
**Answer**: The heavier diamond should be 7765.1 dollars more
